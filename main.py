...
# Usiamo gemini-1.5-flash perché è veloce, multimodale (testo+foto) e rientra nel Free Tier
model = genai.GenerativeModel('gemini-1.5-flash')

# Configurazione di sicurezza per evitare blocchi inutili
safety_settings = [
...
async def generate_summary(content_parts):
    ...
    content_parts può essere solo testo o [testo, immagine].
    """
    try:
        prompt_base = (
            "Sei un assistente per una radio. Analizza il contenuto fornito (testo ed eventuale immagine). "
            "Se c'è un'immagine con testo, trascrivilo mentalmente e usalo per il contesto (OCR). "
            "Genera un riassunto IMPERSONALE (senza citare nomi utenti), BREVE e CONCISO (tra 30 e 50 parole). "
            "Vai dritto al punto."
        )
        
        # Aggiungiamo il prompt alle parti da inviare (immagine o testo messaggi)
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2. LIVE DETECTION (Priorità assoluta)
    if is_live_content(text_content):
        alert_msg = "🔴 LIVE IN CORSO! Contatta @The_Edge_Lab_Italia per partecipare alla diretta in privato"
    ...
    if message.photo:
        try:
            # Prende la foto a più alta risoluzione
...
if __name__ == '__main__':
    ...
    # Esecuzione (run_polling gestisce automaticamente il loop async)
